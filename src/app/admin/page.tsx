      ...
      <main className="mx-auto max-w-3xl px-5 py-6">
        ...
        {pending.length === 0 && (
          <p className="mt-6 rounded-2xl bg-white p-6 text-sm">
            Nada pendiente. Aquí caen las publicaciones de electrónica, que esperan
            revisión hasta que se pueda contrastar el IMEI de forma automática, y
            cualquier publicación que alguien reporte.
          </p>
        )}

        <ul className="mt-5 flex flex-col gap-3">
          {pending.map((l) => (
            <li key={l.id} className="rounded-2xl bg-white p-4">
              <div className="flex items-baseline justify-between gap-4">
                <Link href={`/producto/${l.id}`} className="font-medium underline">
                  {l.title}
                </Link>
                <span className="shrink-0 text-sm">{formatCop(l.price_cop)}</span>
              </div>
              ...
              {l.reports > 0 && (
                <p className="mt-1.5 text-sm text-warn">
                  {l.reports === 1 ? "1 reporte" : `${l.reports} reportes`}: {l.reasons}
                </p>
              )}
