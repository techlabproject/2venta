...
export function RegisterStoreForm() {
  const [result, submit, pending] = useActionState<StoreResult | null, FormData>(
    registerStore,
    null
  );
  ...
  return (
    <form action={submit} className="flex flex-col gap-4">
      ...
      <Field id="legalName" name="legalName" label="Razón social" required
        placeholder="Tecnología Usaquén S.A.S." />
      <Field id="nit" name="nit" label="NIT" required placeholder="899999063-3"
  ...
  return (
    <form action={submit} className="mt-4 flex flex-col gap-3">
      ...
      {result?.created !== undefined && (
        <div role="status" className="rounded-xl bg-brand/10 px-4 py-3 text-sm text-brand">
          ...
          {result.rowErrors && result.rowErrors.length > 0 && (
            <ul data-testid="lote-errores" className="mt-2 flex flex-col gap-1 text-warn">
      ...
      <input type="file" name="file" accept=".csv,text/csv" aria-label="Archivo de artículos"
        required
