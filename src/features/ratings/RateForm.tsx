...
export function RateForm({
  ...
  const [result, submit, pending] = useActionState<RatingResult | null, FormData>(
    rateCounterpart,
    null
  );
        ...
        <fieldset className="flex flex-col gap-1.5">
          ...
          {[5, 4, 3, 2, 1].map((n) => (
            <label key={n} className="flex items-center gap-2 text-sm">
              ...
              <span aria-hidden="true" className="text-accent-text">{"★".repeat(n)}</span>
              <span className="sr-only">{n} de 5</span>
            </label>
          ))}
        </fieldset>

        <textarea name="review" aria-label="Tu reseña" rows={3}
